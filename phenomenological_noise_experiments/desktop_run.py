...
def main(
    ...
    for bias, tasks in bias_tasks.items():
        ...
        samples = sinter.collect(
            ...
            print_progress=True,
            save_resume_filepath=f"./resume_15_3/data.csv",
        )
...
def load_or_create_stim_circuit(
    ...
    #    filepath = output_path() / f"stim_circuits/{hashed}.stim"
    filepath = Path(
        f"./stim_circuits/m_{m}_q_{q}_code_{type(code).__name__}_distance_{code.distance}_layers_{layers}.stim"
    )
    if filepath.is_file():
        stim_circuit = stim.Circuit.from_file(filepath)
    else:
