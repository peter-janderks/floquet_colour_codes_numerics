...
def multi_plot(plot, p_I_list):
    ...
    fig, axs = plt.subplot_mosaic(
        plot_names, sharex=True, figsize=(4, 10), constrained_layout=True
    )
...
def main(plot, p_I, ax):
    samples_normal = sinter.stats_from_csv_files(
        f"./resume_11_2/GaugeFloquetColourCodeX_1_3.csv"
    )
...
def line_fit_plot(ax,):
    ...
    for distance in distances:
        for index, per in enumerate(code_dict[str(distance)]["per"]):
            data_to_plot[float(per)][distance] = float(
                code_dict[str(distance)]["ler"][index]
            )
...
def teraquop_plot(ax, code_dict, distance_to_qubits, case_i, name):
    ...
    for per in sorted(data_to_plot.keys()):
        ...
        if len(distances) > 1:
            ...
            if teraquop_d > 0:
                d_min, d_max = get_min_and_max_distances(lers_filtered, distances, 1e-6)
...
if __name__ == "__main__":
    ...
    multi_plot("line_fit", [1.0, 0.75, 0.5, 0.25, 0.1, 0.05])
# 5
